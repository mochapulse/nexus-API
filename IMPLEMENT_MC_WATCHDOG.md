...
- **`watchdog.STATE_LOCK` removed (simplification pass 2)**: it never protected anything real — this process runs a single asyncio event loop, so any code with no `await` in it (arm/disarm/snapshot/tick, and the HTTP handlers calling them) is already atomic with respect to every other coroutine. The one real race, `watchdog_loop`'s blocking probe/restart/poweroff calls crossing an `await`, was already (and still is) guarded by re-checking `arm_generation` on the other side.
